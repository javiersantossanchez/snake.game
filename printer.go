...
func printFood(food game.Cell) {
	termbox.SetCell(food.Column+1, food.Row+2, ' ', termbox.ColorCyan, termbox.ColorCyan)
}
...
func printWalls(w int, h int) {
	for i := 1; i < h+3; i++ {
		termbox.SetCell(0, i, ' ', termbox.ColorGreen, termbox.ColorGreen) // |
		//termbox.SetCell(1, i, ' ', termbox.ColorGreen, termbox.ColorGreen)
	}

	for i := 1; i < h+3; i++ {
		termbox.SetCell(w+1, i, ' ', termbox.ColorGreen, termbox.ColorGreen) //|
		//termbox.SetCell(9, i, ' ', termbox.ColorGreen, termbox.ColorGreen)
	}

	for i := 0; i < w+2; i++ {
		termbox.SetCell(i, 1, ' ', termbox.ColorGreen, termbox.ColorGreen) // --
	}

	for i := 0; i < w+2; i++ {
		termbox.SetCell(i, h+2, ' ', termbox.ColorGreen, termbox.ColorGreen) // --
	}
